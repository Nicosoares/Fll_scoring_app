...
public class DoLogin extends BaseFLLServlet {
  ...
  public static void doLogin(final HttpServletRequest request,
    ...
    try {
      ...
      // compute hash
      final String tNumber = request.getParameter("tNumber");
      if(null != tNumber) {
            redirect = "login.jsp?tNumber="+tNumber;
      }
      ...
      for (final Map.Entry<String, String> entry : authInfo.entrySet()) {
        if (user.equals(entry.getKey())
          ...
          if(null != tNumber) {
            redirect = "scoreEntry/selected_team.jsp?team="+tNumber+"&referee="+user;
          }
          else {
            redirect = SessionAttributes.getRedirectURL(session);
            if (null == redirect) {
              redirect = "index.jsp";
            }
          }
