...
public class InitFilter implements Filter {
  ...
  public void doFilter(final ServletRequest request,
      ...
      try {
        if (response instanceof HttpServletResponse
          ...
          final boolean needsInit = needsInit(contextPath, path);
          final boolean needsSecurity = needsSecurity(connection, contextPath, path);
  ...
  /**
   * Check if the path requires the user to have a grant to access the resource
   *
   * @param connection database connection
   * @param contextPath the context of the web app
   * @param path the path to the requested resource
   * @return true if grant required, false otherwise
   */
  public static boolean grantRequired(final Connection connection,
    ...
    try {
      prep = connection.prepareStatement("SELECT grant_uri" 
                                       + "  FROM fll_grant");
      ...
      while (rs.next()) {
        ...
        if (e instanceof java.sql.SQLSyntaxErrorException) { /* table fll_grant not created yet */
            return false;
        }
   ...
   */
  private boolean needsSecurity(final Connection connection,
                                final String contextPath,
                                final String path) 
      throws SQLException {
    LOGGER.debug("Checking contextPath: "
        ...
        + path);
    if (null != path
        && grantRequired(connection, contextPath, path)
    /*
  ...
  private boolean checkSecurity(final HttpServletRequest request,
    ...
    if (WebUtils.checkAuthenticated(request, application) && WebUtils.checkAuthorized(request, application)) {
      return true;
    } else {
