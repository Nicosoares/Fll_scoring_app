...
public final class WebUtils {
  ...
  /**
   * Check if the user is authorized to access URI in contextPath + path
   *
   * @param connection database connection
   * @param user the username
   * @param contextPath the context of the web app
   * @param path the path to the requested resource
   * @return true if authorized, false otherwise
   */
  public static boolean userAuthorized(final Connection connection,
                                       final String user, 
    ...
    try {
      prep = connection.prepareStatement("SELECT fll_grant.grant_uri" 
                                       + "  FROM fll_user_data"
                                       + " INNER JOIN fll_permission ON fll_user_data.user_role = fll_permission.role_id"
                                       + " INNER JOIN fll_role ON fll_role.role_id = fll_permission.role_id"
                                       + " INNER JOIN fll_grant ON fll_grant.grant_id = fll_permission.grant_id"
                                       + " WHERE fll_user_data.fll_user = ?");
      prep.setString(1, user);
      rs = prep.executeQuery();
      while (rs.next()) {
        final String uri = rs.getString(1);
        
        if (path.startsWith(contextPath + uri)) {
            return true;
        }
      }
    } catch (final SQLException e) {
   ...
   /**
   * Check if the user doing the web request is authorized to do it.
   * 
   * @param request the web request
   * @param application the application context
   * @return true if authorized, false otherwise
   */
  public static boolean checkAuthorized(final HttpServletRequest request,
                                        final ServletContext application) {
    final DataSource datasource = ApplicationAttributes.getDataSource(application);
    ...
    try {
      ...
      if (Queries.isAuthorizationEmpty(connection)) {
        LOGGER.debug("Returning true from checkAuthorized for empty auth");
        return true;
      }
      ...
      if (userAuthorized(connection, user, contextPath, path)) {
          
        LOGGER.debug("Returning true from checkAuthorized for user logged in: "
            + contextPath + path
            + " user: "
            + user);
        return true;
      } else {
        LOGGER.debug("Returning false from checkAuthorized for user logged in:"
            + contextPath + path
            + " user: "
            + user + " - not authorized");
        return false;
