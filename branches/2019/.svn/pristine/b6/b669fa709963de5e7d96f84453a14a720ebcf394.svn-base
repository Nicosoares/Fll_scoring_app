...
public final class ScoreEntry {
    ...
    for (final AbstractGoal goal : performanceElement.getGoals()) {
      if (goal.isComputed()) {
        ...
        if (!goal.isEnumerated()
          ...
          formatter.format("  var temp = %s%n", rawVarName);
          formatter.format("  %s = increment;%n", rawVarName); //"+=" changed into "=" due to Slider use
  ...
  public static void generateRefreshBody(final Writer writer,
    ...
    for (final AbstractGoal agoal : performanceElement.getGoals()) {
      if (agoal.isComputed()) {
        ...
        if (goal.isEnumerated()) {
          ...
          // set the slider value (used for printing) to match the gbl variable
          formatter.format("$('#%s_slider_value').text(%s);%n", name, rawVarName);
          // set the count form element
          formatter.format("document.scoreEntry.%s.value = %s;%n", name, rawVarName);
          formatter.format("$('[data-slider-id=%s_slider]').slider({ min: %s, max: %s, value: %s, focus: true });%n", name, min, max, rawVarName); // does not set data-slider-value attribute
          // set the slider value and data-value attributes to match the gbl variable
          formatter.format("$('#%s').slider('setValue', %s);%n", name, rawVarName);
          formatter.format("%s = %s * %s;%n", computedVarName, rawVarName, multiplier);
        }
        ...
        formatter.format("changePanelStyle( '#panel_%s', %s, false);%n", name, rawVarName);
        formatter.format("%n");
      }
    } // end foreach goal
    ...
    formatter.format("  $( '#score_verification' ).removeClass( 'panel panel-success' ).addClass( 'panel panel-danger' );%n");
    formatter.format("} else {%n");
    formatter.format("  document.scoreEntry.Verified[1].checked = true;%n"); // YES
    formatter.format("  $( '#score_verification' ).removeClass( 'panel panel-danger' ).addClass( 'panel panel-success' );%n");
    formatter.format("}%n");
  ...
  public static void generateCheckRestrictionsBody(final Writer writer,
    ...
    for (int restrictIdx = 0; restrictIdx < restrictions.size(); ++restrictIdx) {
      ...
      for (final Term term : restrictEle.getTerms()) {
        for (final GoalRef goalRef : term.getGoals()) {
          final String goalName = goalRef.getGoalName();
          final String rawVarName = getVarNameForRawScore(goalName);
          formatter.format("    changePanelStyle( '#panel_%s', %s, true);%n", goalName, rawVarName);
        }
      }
      formatter.format("  }%n");
    }
  ...
  public static void generateVerificationInput(final JspWriter writer) throws IOException {
    ...
    writer.println("    <div class='panel panel-danger hidden-print' id='score_verification'>");
    writer.println("      <div class='panel-heading'>Score entry verified:</div>");
  ...
  public static void generateScoreEntry(final JspWriter writer,
    ...
    writer.println("<div class='panel-group' id='accordion'>"); //Missing </div> is further down
...
*/
        if (++count % 2 == 1) {
           printColumn = "printcolumn-left";
        }
        else {
           printColumn = "printcolumn-right";
        }
        writer.println("<!-- " + name + " -->");
        writer.println("<div class='panel panel-default " + printColumn + "' id='panel_" + name + "'>"); //Missing </div> is further down
        writer.println("    <div class='panel-heading'>");
        writer.println("        <h4 class='panel-title'>");
        writer.println("            <a data-toggle='collapse' data-parent='#accordion' href='#collapse_"+ name + "'>" + title + "</a>");
        writer.println("        </h4>");
        writer.println("    </div>");
        
        writer.println("    <div id='collapse_"+ name + "' collapse-id='" + name + "_collapse'" + " class='panel-collapse collapse'>"); //Missing </div> is further down
        writer.println("    <div class='panel-body'>"); //Missing </div> is further down

        if (goalEle.isComputed()) {
          writer.println("  <div><b>Computed Goal</b></div>");
        } else {
        ...
        // computed score
        writer.println("  <div>");
        writer.println("    <input type='hidden' name='score_"
            + name + "' size='3' align='right' readonly tabindex='-1'>");
        writer.println("  </div>");

		writer.println("  </div>"); //Closes panel-body
        writer.println("</div>"); //Closes panel-collapse
        writer.println("</div>"); //closes panel-default
        writer.println("<!-- end "
            + name
            + " -->");
        writer.newLine();
    ...
    writer.println("</div>"); //Closes panel-group
  }
...
*/
	  writer.println("<div class='visible-print-inline'><span id='" + name + "_slider_value' >0</span></div>" + 
        "<div class='hidden-print'>" +
        "<input id='" + name + "' name='" + name + "' data-slider-id='" + name + "_slider' type='text' data-slider-min='" + 
		min + "' data-slider-value='0' data-slider-max='" + 
		max + "' data-slider-step='1' data-provide='slider' " + 
        generateSliderTicks(min, max) + 
        " onchange='" + getIncrementMethodName(name) + "(parseInt(this.value))' /></div>");
    }
    ...
    if (FP.equals(0, min, ChallengeParser.INITIAL_VALUE_TOLERANCE)
      ...
      writer.println("    <input type='hidden' name='"
          + name
          + "_sliderValue' size='3' align='right' onChange='"
  ...
  public static String generateSliderTicks(final double min,
                                           final double max) {
    String sliderTicks = "data-slider-ticks='[";
    String sliderTicksLabels = "data-slider-ticks-labels='[";
    
    for (int i=(int) Math.floor(min); i<(int) Math.ceil(max); i++) {
        sliderTicks += String.valueOf(i) + ", ";
        sliderTicksLabels += "\"" + String.valueOf(i) + "\", ";
    }
    
    sliderTicks += String.valueOf((int) Math.ceil(max)) + "]'";
    sliderTicksLabels += "\"" + String.valueOf((int) Math.ceil(max)) + "\"]'";

    return sliderTicks + " " + sliderTicksLabels;
  }
